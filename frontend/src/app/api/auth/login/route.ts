...
export async function GET(request: Request) {
  return NextResponse.redirect(new URL("/pt-BR/entrar", request.url), { status: 303 });
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.redirect(new URL(`/${fallbackLocale}/entrar?error=1`, request.url), { status: 303 });
  }
}
