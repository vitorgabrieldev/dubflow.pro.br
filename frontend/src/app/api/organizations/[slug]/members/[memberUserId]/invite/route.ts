...
export async function DELETE(
  ...
  const cancelInvite = async (authToken: string) =>
    fetch(`${API_BASE_URL}/organizations/${slug}/members/${memberUserId}/invite`, {
      method: "DELETE",
      headers: {
  ...
  let response = await cancelInvite(token);
  ...
  if (response.status === 401) {
    ...
    if (refreshResponse.ok) {
      ...
      if (refreshPayload.access_token) {
        ...
        response = await cancelInvite(token);
      }
    }
  }

  const result = await response.json().catch(() => ({ message: "Não foi possível anular o convite." }));
