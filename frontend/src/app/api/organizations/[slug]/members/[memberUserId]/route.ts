...
export async function DELETE(
  ...
  const removeMember = async (authToken: string) =>
  ...
  let response = await removeMember(token);
  ...
  if (response.status === 401) {
    ...
    if (refreshResponse.ok) {
      ...
      if (refreshPayload.access_token) {
        ...
        response = await removeMember(token);
      }
    }
  }

  const result = await response.json().catch(() => ({ message: "Não foi possível remover." }));
