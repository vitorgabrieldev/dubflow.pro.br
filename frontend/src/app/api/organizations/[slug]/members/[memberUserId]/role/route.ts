...
export async function PATCH(
  request: Request,
  context: { params: Promise<{ slug: string; memberUserId: string }> }
) {
  const { slug, memberUserId } = await context.params;
  ...
  if (!body.role) {
    return NextResponse.json({ message: "Cargo inválido." }, { status: 422 });
  }
  ...
  const updateRole = async (authToken: string) =>
    fetch(`${API_BASE_URL}/organizations/${slug}/members/${memberUserId}`, {
      method: "PATCH",
      headers: {
  ...
  let response = await updateRole(token);
  ...
  if (response.status === 401) {
    ...
    if (refreshResponse.ok) {
      ...
      if (refreshPayload.access_token) {
        ...
        response = await updateRole(token);
      }
    }
  }

  const result = await response.json().catch(() => ({ message: "Não foi possível atualizar o cargo." }));
