...
export async function POST(
  ...
  const banMember = async (authToken: string) =>
    fetch(`${API_BASE_URL}/organizations/${slug}/members/${memberUserId}/ban`, {
      method: "POST",
      headers: {
  ...
  let response = await banMember(token);
  ...
  if (response.status === 401) {
    ...
    if (refreshResponse.ok) {
      ...
      if (refreshPayload.access_token) {
        ...
        response = await banMember(token);
      }
    }
  }

  const result = await response.json().catch(() => ({ message: "Não foi possível banir o membro." }));
