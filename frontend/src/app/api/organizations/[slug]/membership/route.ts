...
async function proxyMembership(slug: string, method: "POST" | "DELETE") {
  ...
  const endpoint = method === "POST" ? "join-request" : "leave";

  let response = await fetch(`${API_BASE_URL}/organizations/${slug}/${endpoint}`, {
    method,
    headers: {
  ...
  if (response.status === 401) {
    ...
    if (refreshResponse.ok) {
      ...
      if (refreshPayload.access_token) {
        ...
        response = await fetch(`${API_BASE_URL}/organizations/${slug}/${endpoint}`, {
          method,
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const payload = await response.json().catch(() => ({
    message: "Falha ao atualizar a participação na comunidade.",
  }));
