...
async function proxyFollow(userId: string, method: "POST" | "DELETE") {
  ...
  let response = await fetch(`${API_BASE_URL}/users/${userId}/follow`, {
    method,
    headers: {
  ...
  if (response.status === 401) {
    ...
    if (refreshResponse.ok) {
      ...
      if (refreshPayload.access_token) {
        ...
        response = await fetch(`${API_BASE_URL}/users/${userId}/follow`, {
          method,
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const payload = await response.json().catch(() => ({
    message: "Falha ao atualizar o acompanhamento do usuário.",
  }));
