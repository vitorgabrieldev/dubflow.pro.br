...
const config = {
	title            : "Playlists",
	permissionPrefix : "playlists",
	list             : "playlists-admin",
	searchPlaceholder: "Buscar por id, título, slug ou descrição",
	orders           : [
...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{ title: "Ano", className: "text-center", render: (item) => item.release_year || "-" },
			{ title: "Visibilidade", className: "no-ellipsis", render: (item) => <Tag color={item.visibility === "public" ? "#0acf97" : "#f7b84b"}>{item.visibility}</Tag> },
			{ title: "Posts", className: "text-center", render: (item) => item.posts_count ?? 0 },
			{
				title    : "Criação",
				className: "datetime",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		playlistsAdminService.getAll(data)
		.then((response) => {
			if( exportItems ) {
				this.setState({isExporting: false});
				downloadPrivateFile(response.data.file_url, "playlists.csv").catch((error) => Modal.error({title: "Ocorreu um erro!", content: String(error)}));
			} else {
				this.setState(state => ({
					isLoading: false,
	...
	editOpen = ({uuid}) => {
		Modal.confirm({
			...
			content: "Alterar dados de playlists de terceiros pode impactar publicações existentes. Deseja continuar?",
			onOk: () => {
	...
	deleteConfirm = ({uuid, title}) => Modal.confirm({title: "Confirmar exclusão", content: `Tem certeza de que deseja remover (soft delete) a playlist ${title}?`, okText: "Excluir", onOk: () => this.deleteConfirmed(uuid)});
	deleteConfirmed = (uuid) => playlistsAdminService.destroy({uuid}).then(() => this.fetchGetAll()).catch((data) => Modal.error({title: "Ocorreu um erro!", content: String(data)}));
