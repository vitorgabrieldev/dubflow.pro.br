...
const config = {
	...
	permissionPrefix : "customers-deleted",
...
class Index extends Component {
	...
	menuItem = (item) => (
		<Menu className="actions-dropdown-menu">
			{this.hasPermission(config.permissionPrefix + ".show", "customers.show") && (
				<Menu.Item key="show">
	...
	columns = () => {
		...
		return [
			...
			},
			{
				...
				render   : (item) => {
					const formatted = this.formatDateTime(item?.deleted_at);
			...
			},
			{
				...
				visible  : this.hasPermission(config.permissionPrefix + ".show", "customers.show"),
				render   : (item) => (
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		customerDeletedService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
			...
			<QueueAnim className="site-content-inner">
				<div className="page-content" key="1">
					...
					<UIPageListing
						...
						buttons={[
							{
								visible: this.hasPermission(config.permissionPrefix + ".export", "customers.export"),
