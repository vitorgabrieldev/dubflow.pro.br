...
import { pushCityService } from "./../../redux/services";
...
const config = {
	title            : "Push por cidade",
	permissionPrefix : "push-city",
	list             : "push-city",
...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Cidade",
				render: (item) => <span title={item.city?.name ?? '-'}>{item.city.name ?? '-'}</span>,
			},
			{
				title : "Título",
	...
	deleteConfirmed = (uuid) => {
		return pushCityService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
