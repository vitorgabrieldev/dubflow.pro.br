...
import { vehicleModelsService } from "./../../redux/services";
...
const config = {
	...
	permissionPrefix : "modelosveiculos",
	list             : "modelosveiculos",
...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Marca",
				render: (item) => listTypeCard ? <h3>{item.marca?.name}</h3> : item.marca?.name,
			},
			{
				title : "Nome",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		vehicleModelsService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return vehicleModelsService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
