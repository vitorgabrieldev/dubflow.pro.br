...
const config = {
	...
	list             : "faq",
	searchPlaceholder: "Buscar por texto da pergunta e resposta",
	orders           : [
		{
			label  : "Mais recentes",
			field  : "created_at",
...
class Index extends Component {
	...
	getQuestion = (item) => item?.question ?? item?.name ?? "N/A";

	getAnswer = (item) => item?.answer ?? item?.text ?? "";

	getAnswerPreview = (item) => {
		const text = stripHtml(this.getAnswer(item));
		return text || "N/A";
	};
	...
	columns = () => {
		...
		return [
			...
			},
			{
				...
				render: (item) => listTypeCard ? <h3>{normalizeUserType(item?.user_type ?? item?.type)}</h3> : normalizeUserType(item?.user_type ?? item?.type),
			},
			{
			...
			},
			{
				...
				render: (item) => {
					const answer = this.getAnswerPreview(item);
			...
			},
			{
				title    : "Ordem",
				width    : 110,
				className: "text-center card-block",
				render   : (item) => {
					const order = typeof item?.order === "undefined" || item?.order === null ? "N/A" : item.order;

					if( listTypeCard )
					{
						return (
							<Fragment>
								<span className="value">{order}</span>
								<label>Ordem</label>
							</Fragment>
						);
					}
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		faqService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
	...
	deleteConfirmed = (uuid) => {
		return faqService.destroy({uuid})
		.then(() => {
			this.fetchGetAll();
		})
		.catch((data) => {
