...
const config = {
	title            : "Push por estado",
	permissionPrefix : "push-state",
	list             : "push-state",
...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Estado",
				render: (item) => <span title={item.state && item.state.name}>{item.state && item.state.name}</span>,
			},
			{
				title : "Título",
	...
	deleteConfirmed = (uuid) => {
		return pushStateService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
