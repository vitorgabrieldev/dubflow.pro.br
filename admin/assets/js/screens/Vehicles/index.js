...
class Index extends Component {
	...
	deleteConfirmed = (uuid) => {
		return vehiclesService.destroy({uuid})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
	...
	disabledConfirm = (uuid) => {
		Modal.confirm({
			title          : "Confirmação de desativação de veículo",
			content        : "Tem certeza de que deseja desativar este veículo?",
			okText         : "Confirmar",
			autoFocusButton: null,
			onOk           : () => {
	...
	disabled = (uuid) => {
		...
		vehiclesService.chargeStatus({uuid: uuid, is_active: false})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
	...
	enabledConfirm = (uuid) => {
		Modal.confirm({
			title          : "Confirmação de ativação de veículo",
			content        : "Tem certeza de que deseja ativar este veículo?",
			okText         : "Confirmar",
			autoFocusButton: null,
			onOk           : () => {
	...
	enabled = (uuid) => {
		...
		vehiclesService.chargeStatus({uuid: uuid, is_active: true})
		.then((response) => {
			// Fecth all
			this.fetchGetAll();
		})
		...
		<Menu className="actions-dropdown-menu">
			{this.props.permissions.includes(config.permissionPrefix + ".edit") && <Menu.Item key="disabled" className="divider">
				<a onClick={() => {{!item.is_active ? this.enabledConfirm(item.uuid) : this.disabledConfirm(item.uuid);}}}>
					<i className="fal fa-toggle-on" /> {!item.is_active ? "Ativar veículo" : "Desativar veículo"}
				</a>
			</Menu.Item>}
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Profissional",
				render: (item) => item.profissional?.name || 'N/A',
			},
			{
				title : "Marca",
				render: (item) => item.marca?.name || 'N/A',
			},
			{
				title : "Modelo",
				render: (item) => item.modelo?.name || 'N/A',
			},
			{
				title : "Tipo",
				render: (item) => {
			...
			},
			{
			  ...
			  visible  : this.props.permissions && this.props.permissions.includes(config.permissionPrefix + ".show"),
			  render   : (item) => (
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		{
			data.marca_id = filters.marca_id;
		}

		if( filters.modelo_id !== null )
		{
			data.modelo_id = filters.modelo_id;
		}
