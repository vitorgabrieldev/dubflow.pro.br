...
const config = {
	...
	searchPlaceholder: "Buscar por id, tipo, título ou usuário",
	orders           : [
		{ label: "Mais recentes", field: "created_at", sort: "desc", default: true },
		{ label: "Mais antigas", field: "created_at", sort: "asc" },
	],
};
...
class Index extends Component {
	...
	menuItem = (item) => (
		<Menu className="actions-dropdown-menu">
			...
			{this.props.permissions.includes(config.permissionPrefix + ".delete") && <Menu.Item key="delete" className="divider btn-delete"><a onClick={() => this.deleteConfirm(item)}><i className="fal fa-trash" />Excluir</a></Menu.Item>}
		</Menu>
	);
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		if( filters.is_read !== null ) data.is_read = filters.is_read;
		...
		notificationsAdminService.getAll(data)
		.then((response) => {
			if( exportItems ) {
				this.setState({isExporting: false});
				downloadPrivateFile(response.data.file_url, "notificacoes.csv").catch((error) => Modal.error({title: "Ocorreu um erro!", content: String(error)}));
			} else {
				this.setState(state => ({
					isLoading: false,
	...
	deleteConfirm = ({uuid}) => Modal.confirm({title: "Confirmar exclusão", content: "Tem certeza de que deseja remover esta notificação?", okText: "Excluir", onOk: () => this.deleteConfirmed(uuid)});
	deleteConfirmed = (uuid) => notificationsAdminService.destroy({uuid}).then(() => this.fetchGetAll()).catch((data) => Modal.error({title: "Ocorreu um erro!", content: String(data)}));
