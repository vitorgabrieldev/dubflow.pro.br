...
const config = {
	title            : "Categorias dos serviços",
	permissionPrefix : "service_categories",
...
class Index extends Component {
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Título",
				render: (item) => listTypeCard ? <h3>{item.title || "N/A"}</h3> : (item.title || "N/A"),
			},
			{
				title    : "Criação",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		serviceCategoriesService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
