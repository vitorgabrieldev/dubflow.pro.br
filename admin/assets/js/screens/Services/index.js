...
class Index extends Component {
	...
	getCategoryLabel = (item) => {
		return item?.service_category?.title
	...
	getTypeLabel = (item) => {
		return item?.service_type?.title
	...
	columns = () => {
		...
		return [
			...
			{
				title : "Categoria",
				render: (item) => this.getCategoryLabel(item),
			},
			{
				title : "Tipo",
				render: (item) => this.getTypeLabel(item),
			},
			{
				title : "Nome do serviço",
				render: (item) => listTypeCard ? <h3>{this.getServiceName(item)}</h3> : this.getServiceName(item),
			},
			{
				title : "Valor",
				render: (item) => this.formatCurrency(item?.price ?? item?.value),
			},
			{
				title    : "Criação",
	...
	fetchGetAll = (init = false, exportItems = false) => {
		...
		if( filters.service_category_id !== null )
		{
			data.service_category_id = filters.service_category_id;
		}

		if( filters.service_type_id !== null )
		{
			data.service_type_id = filters.service_type_id;
		}
		...
		servicesService.getAll(data, this._cancelToken.token)
		.then((response) => {
			if( exportItems )
			{
				this.setState({
